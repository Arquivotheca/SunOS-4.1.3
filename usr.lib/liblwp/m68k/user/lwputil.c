...
#include "schedule.h"
#ifndef lint
SCCSID(@(#) lwputil.c 1.1 92/07/30 Copyr 1986 Sun Micro);
#endif lint
 ...
 * lwp_fpset(tid) -- enable fp context switching for a given lwp
...
#define	FPSAVESIZE	256	/* from /usr/src/lib/libc/crt/fpcrtdefs.h */
typedef struct fp_ctxt_t {
	char fp_state[FPSAVESIZE];
} fp_ctxt_t;
STATIC int FpCtx;
	...
	thread_t new;
{
	 ...
	 */
	if ((int *)((lwp_t *)old.thread_id)->lwp_context[SP] < k) {
		__panic("stack overflow"); 
	}

}

/* enable fp special contexts */
void
lwp_fpenable()
{
	extern void __fp_save();		/* in low.s */
	extern void __fp_restore();		/* in low.s */

	FpCtx = lwp_ctxset(__fp_save, __fp_restore, sizeof (fp_ctxt_t), TRUE);
}

/* 
 * lwp_fpset -- PRIMITIVE.
 * set a lwp to have floating point context
 * XX should auto config this if can tell that a thread uses fp.
 */
int
lwp_fpset(tid)
	thread_t tid;
{
	(void)lwp_ctxinit(tid, FpCtx);
}
