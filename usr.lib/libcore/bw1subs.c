 ...
 */
#ifndef lint
static char sccsid[] = "@(#)bw1subs.c 1.1 92/07/30  Copyr 1985 Sun Micro";
#endif
...
/*--------------------------------------------------------------------------*/
_core_bw1simline( x0,y0,x1,y1,wind,op,width,style)
...
short x0,y0,x1,y1;
{
    ...
    (void)pr_vector(destpr, xb>>10, yb>>10, x1, y1,rop,1);
    /* do rest of line */
}
...
/*		This routine provides for plotting characters on the
	view surfaces in two dimensions.  The fonts have fill vectors so
	that the characters will appear solid if plotted at standard size.  */	
/****************************************************************************/

_core_bw1vectext( s, font, up, path, space, x0, y0, vwprt, op, windptr)
    char *s; short font;
