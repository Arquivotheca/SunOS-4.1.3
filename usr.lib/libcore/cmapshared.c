 ...
 */
#ifndef lint
static char sccsid[] = "@(#)cmapshared.c 1.1 92/07/30 SMI 1986";
#endif
...
static PFI wmgr_errorcached;
extern PFI win_errorhandler();
    ...
    struct cms_map *dcmap;
{
	 ...
	 * Open window. 
	 */
	(void)win_numbertoname(winnumber, winname);
	 ...
	 */
	wmgr_errorcached = win_errorhandler(screenget_error);
	windownotactive = 0;
	(void)win_screenget(testwfd, &newscreen);
	if (windownotactive) {
	    (void)close(testwfd);
	    continue;
	}
	...
	if (strncmp(screen.scr_fbname, newscreen.scr_fbname,
	     ...
	     */
	    (void)win_getcms(testwfd, &scms, &cmap);
	    if (strncmp(name, scms.cms_name, CMS_NAMESIZE) == 0) {
		(void)win_getcms(testwfd, &scms, dcmap);
		if (dcms)
		    *dcms = scms;
		satisfied = 1;
		(void)close(testwfd);
