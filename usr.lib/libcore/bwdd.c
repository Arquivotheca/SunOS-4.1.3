...
static char sccsid[] = "@(#)bwdd.c 1.1 92/07/30 SMI";
#endif
...
/*
 * Generic monochrome dd routine for SunCore
 */
...
_core_bwdd(ddstruct, fbtype, fbname)
	...
	char *fbname;
{
	...
	case LINE:
		...
		(void)_core_bw1simline(ip->ddcp[0], ip->screen->pr_height - 1 - ip->ddcp[1], i1, ip->screen->pr_height - 1 - i2,
	...
	case VTEXT:		/* vector text */
	    ...
	    (void)_core_bw1vectext(ddstruct->ptr1, ip->ddfont, &ip->ddup,
