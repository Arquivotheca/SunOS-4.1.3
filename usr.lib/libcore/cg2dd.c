 ...
 */
#ifndef lint
static char sccsid[] = "@(#)cg2dd.c 1.1 92/07/30 Copyr 1985 Sun Micro";
#endif
...
cg2dd(ddstruct) register ddargtype *ddstruct;
{
	...
	case INITIAL: {
		...
		if (*surfp->vsurf.screenname != '\0') {
			...
			fbfd = _core_getfb(cgtwodev,FBTYPE_SUN2COLOR, ddstruct->int1);
			if (fbfd >= 0)
				(void)strncpy(screen.scr_fbname, cgtwodev, SCR_NAMESIZE);
			else {
			    ddstruct->logical = FALSE;
			    return( 1);
			    }
