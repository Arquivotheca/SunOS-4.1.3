...
#ifndef lint
static  char sccsid[] = "@(#)i_video_mem_i860_depth.c 1.1 92/07/30 Copyr 1990 Sun Micro";
#endif
...
/**********************************************************************/
void
i_video_mem_i860_depth(ctxp, umcbp, kmcbp, fputs) 
/**********************************************************************/
...

{
	...
	txdat[0] = 'D';txdat[1] = 'e';txdat[2] = 'p';txdat[3] = 't';
	txdat[4] = 'h';
	txdat[5] = '-';txdat[6] = 'E';txdat[7] = 'r';txdat[8] = 'r';
	txdat[9] = ':';txdat[10] = ' ';txdat[11] = '\0';
	...
	/* initialize pp registers */
        ...
        *reg = 5;        /* set FB Addr Select (ASR) reg = 4,5 or 6 */
        reg = (unsigned *)(BUF_SR);
        *reg = 1;    /* set FB buffer select reg = 1 or 6 */

	/* write all memory with (true) address pattern */

	for (y = 0; y < 1024; y++) {
	   addr = (unsigned int *)(VIDEO_MEM_START + (y << 13));
	   for ( x = 0; x < 1280; x++) {	
	      wr_data = (unsigned int)addr & 0xffffff;
	      *addr++ = wr_data;
	   }
	}

	/* verify all memory with (true) address pattern */
	...
	for (y = 0; y < 1024; y++) {
	   ...
	   for ( x = 0; x < 1280; x++) {	
	      ...
	      if (rd_data != wr_data)
		 goto error;
	      *addr++ = ~wr_data & 0xffffff;	/* write back complemented pattern */
	   }
	}

	/* verify all memory with (complemented) address pattern */
