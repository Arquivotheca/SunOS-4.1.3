...
#ifndef lint
static  char sccsid[] = "@(#)i_video_mem_i860_image_b.c 1.1 92/07/30 Copyr 1990 Sun Micro";
#endif
...
/**********************************************************************/
void
i_video_mem_i860_image_b(ctxp, umcbp, kmcbp, fputs) 
/**********************************************************************/
...

{
	...
	txdat[0] = 'I';txdat[1] = 'M';txdat[2] = 'G';txdat[3] = 'B';
	...
	/* initialize pp registers */
        ...
        *reg = 4;        /* set FB Addr Select (ASR) reg = 4,5 or 6 */
        reg = (unsigned *)(BUF_SR);
        *reg = 6;    /* set FB buffer select reg = 1 or 6 */
	...
	for (y = 0; y < 1024; y++) {
	   ...
	   for ( x = 0; x < 1280; x++) {	
	      ...
	      if (rd_data != wr_data)
		 goto error;
	      *addr++ = ~wr_data;	/* write back complemented pattern */
	   }
	}
