...

{
	...
	if (i != not_gr && groups[i] != 0 && (not_gr != PIXPG_24BIT_COLOR || i != PIXPG_8BIT_COLOR)) {
		...
		case PIXPG_24BIT_COLOR :
		    ...
		    break;
		case PIXPG_WID :
		    mask = 0x3f3f3f3f;   /* 6-bit WID */
		    break;
		case PIXPG_ZBUF :
		    mask = 0xffffffff;	 /* 16-bit ZBUFF */
		    break;
		default :
...
/* clear all available pixrect planes */
{
    ...
    wid.wa_count = 1;
    if (pr_ioctl(pr, FBIO_WID_ALLOC, &wid) < 0) {
	(void)send_message(SKIP_ERROR, WARNING, errmsg_list[50]);
    }
    pr_set_planes(pr, PIXPG_WID, PIX_ALL_PLANES);
    op = PIX_SRC | PIX_COLOR(wid.wa_index);
    (void)pr_clear(pr, op);
