...
#ifndef lint
static	char sccsid[] = "@(#)y2.c 1.1 92/07/30 SMI"; /* from S5R3 1.10 */
#endif
...
char * parser=PARSER;	/* source for yyparse() */
...
{	int i,j,lev,t, ty;
	...
	while( argc >= 2  && argv[1][0] == '-' ) {
		...
		while(*++cp) {
			...
			case 'P':
				/* -P<parser> specifies an alternative
				   to /usr/lib/yaccpar, for testing */
				parser = ++cp;
				while (*++cp);
				cp--;
				continue;
	...
	lkst = (struct looksets *) malloc(lsetsize * sizeof(struct looksets));
	...
	/*
	 * yaccpar now uses malloc() and realloc() - declare them as
	 * early as possible, because people who either forget to declare
	 * them or declare them incorrectly are probably too stupid to
	 * figure out that "/usr/lib/yaccpar, line xxx: malloc redefined"
	 * is really their bug.
	 */
	fprintf( ftable,  "extern char *malloc(), *realloc();\n");
	cnamp = cnames;
	defin(0,"$end");
	extval = 0400;
	defin(0,"error");
