...
#define UCASE(c) ( (('a' <= c) && (c <= 'z')) ? c - 32 : c )
				...
				NEXT;
				is = 1;
				while ((UCASE(CURRENT) == infstring[is]) && (is <= 8)) {
					NEXT;
					is++;
				}
				if (UCASE(CURRENT) == infstring[is])
					iagree = is + 1;
				else
					iagree = is;
				is++;	/* To account for infstring indexing
					 * starting at 0. */
		...
		case 'N':
			{	/* Try NaN. */
				...
				NEXT;
				is = 1;
				while (UCASE(CURRENT) == nanstring[is]) {
					NEXT;
					is++;
				}
