...
#ifndef lint
static char sccsid[] = "@(#)restore.c 1.1 92/07/30 SMI"; /* from UCB 5.4 5/22/88 */
#endif not lint
	...
	int type;
{
	...
	if (BIT(ino, dumpmap) == 0) {
		dprintf(stdout, "%s: not on the volume\n", name);
		return (descend);
	}
	if (!mflag) {
...
#		define MODECHG	0x8	/* mode of inode changed */
	 ...
	 */
	case NIL:
		fprintf(stderr, "%s: (inode %d) not found on volume\n",
			name, ino);
		break;
	...
	char *symtabfile;
{
	...
	while (curfile.ino < maxino) {
		...
		while (first < curfile.ino) {
			...
			if (ep == NIL)
				panic("%d: bad first\n", first);
			fprintf(stderr, "%s: not found on volume\n",myname(ep));
		...
		if (ep == NIL)
			panic("unknown file on volume\n");
		if ((ep->e_flags & (NEW|EXTRACT)) == 0)
			badentry(ep, "unexpected file on volume");
		/*
