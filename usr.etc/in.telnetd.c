...
#ifndef lint
static  char *sccsid = "@(#)in.telnetd.c 1.1 92/07/30 SMI"; /* from UCB 5.18 5/12/86 */
#endif not lint
	...
	struct sockaddr_in *who;
{
	...
	/* 
	 * Find an unused pty.  A security hole exists if we allow any
	 * other process to hold the slave side of the pty open at the
	 * time we open it.  The other process could read from the
	 * pty, snarfing up user input such as passwords.  This
	 * problem would not exist if the pty subsystem would fail the
	 * open of the master side when there are existing open
	 * instances of the slave side.  Until the pty driver is
	 * changed to work this way, we use a side effect of one
	 * particular ioctl to test whether any process has the slave
	 * open.
  	 */
