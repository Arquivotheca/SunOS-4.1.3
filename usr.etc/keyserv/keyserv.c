#ifndef lint
static char sccsid[] = "@(#)keyserv.c 1.1 92/07/30 Copyr 1990 Sun Micro";
#endif
/*
...

{
	...
	extern int optind;
	int c;	
	 while ((c = getopt(argc, argv, "nkdD")) != -1)
                    switch (c) {
			case 'n':
				nflag++;
				break;
			...
			default:
				(void) fprintf(stderr, "usage: %s [-n] [-k] [-d]\n", argv[0]);
				(void) fprintf(stderr, "-n prompts for root password\n-k saves keys across reboots\n-d disables the use of default keys\n");
				exit(1);
	...
	int prompt;
{
	...
	if (!prompt) {
		...
		/*
		 * Decrypt NIS entry to get secret key 
		 */
