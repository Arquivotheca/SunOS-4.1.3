#ifndef lint
static	char sccsid[] = "@(#)zic.c 1.1 92/07/30 SMI"; /* from Arthur Olson's 4.14 */
#endif /* !defined lint */
...
typedef int		qsort_size_t;	/* unsigned for S5 */
...
extern int	mkdir P((const char * name, int mode));
...
char *	argv[];
{
	...
	progname = argv[0];
	while ((c = getopt(argc, argv, "d:l:p:vs")) != EOF)
		switch (c) {
			default:
				usage();
			case 'd':
