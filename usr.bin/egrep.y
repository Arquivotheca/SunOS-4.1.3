...
%{
static	char sccsid[] = "@(#)egrep.y 1.1 92/07/30 SMI"; /* from UCB 4.4 5/29/85 */
#include <stdio.h>
#include <ctype.h>
#include <locale.h>
#include <sys/types.h>
	...
	for (s=0; s<=n; s++)  {
		...
		if (iflag) {
			for (c=0; c<NCHARS; c++) {
				if (isupper(c))
					gotofn[s][c] =
					    gotofn[s][(unsigned char)tolower(c)];
			}
		}
	}
}
...
char *file;
{
	...
	p = buf;
	nlp = p;
	if ((ccount = read(f, (char *)p, blksize)) < 0)
		goto readerr;
	else if (ccount == 0)
		goto done;
	in_line = 1;
		...
		found:	for(;;) {
				...
				succeed:	nsucc = 1;
						if (cflag) tln++;
							...
							if (nfile > 1 && hflag) (void) printf("%s:", file);
							if (bflag) (void) printf("%d:", blkno);
							if (nflag) (void) printf("%ld:", lnum);
							if (p <= nlp) {
								while (nlp < &buf[2*blksize]) (void) putchar((char)*nlp++);
								nlp = buf;
							}
