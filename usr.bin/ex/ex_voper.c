...
#ifndef lint
static	char *sccsid = "@(#)ex_voper.c 1.1 92/07/30 SMI"; /* from S5R3.1 1.13 */
#endif
...
#define	blank()		isspace((unsigned char)wcursor[0])
	...
	register int c, cnt;
{
	...
	register line *addr;
	line *odot;
	static unsigned char lastFKND, lastFCHR;
	short d;
#ifdef PTR_ADDRESSES
	int mouse_x;
	int mouse_y;
...
slerr:
		...
		if (*globp == 'z')
			globp++, c = '\n';
		if (any(*globp, "^+-."))
			c = (unsigned char)*globp++;
		i = 0;
		while (isdigit((unsigned char)*globp))
			i = i * 10 + *globp++ - '0';
		if (any(*globp, "^+-."))
			c = *globp++;
		if (*globp) {
	...
	register int (*op)();
{
	...
	if (op != vyankit && op != vchange && op != vdelete && wcursor > linebuf)
		wcursor--;
}
