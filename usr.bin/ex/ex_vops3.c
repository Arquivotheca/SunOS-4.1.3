...
#ifndef lint
static	char *sccsid = "@(#)ex_vops3.c 1.1 92/07/30 SMI"; /* from S5R3.1 1.7 */
#endif
	...
	register char *parens;
{
	...
	if (*parens && !*wcursor && !lnext())
		return (0);
	while (isspace((unsigned char)*wcursor) || (*wcursor == 0 && *parens))
		if (!lnext())
			return (0);
	if (any(*wcursor, parens) || dir > 0)
		return (1);
	for (cp = wcursor; cp > linebuf; cp--)
		if (isspace((unsigned char)cp[-1]) || any(cp[-1], parens))
			break;
	wcursor = cp;
	return (1);
}
