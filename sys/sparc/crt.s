 ...
 *		C	always cleared
 *
 * These routines are called from trap.c as follows
 * _ip_XXXXX(rs1, rs2, dest, &rp->r_y, &rp->r_psr)
...
divout:
	st	%o0,[%i2]	! store the result
	mov	SIMU_SUCCESS, %i0		! success
	ret
	restore
...
/*
 * called as follows
 * _ip_divcc(rs1, rs2, getregaddr(rgs, rw, rd),
 *					&rp->r_y, &rp->r_psr));
 */
	ENTRY(_ip_divcc)
	tst	%o1
...
/* 
 * .long_udivL,H,D)	unsigned 64/32 bits divided
