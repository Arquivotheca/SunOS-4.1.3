 ...
 *
 */
extern int maxunreaped;
extern int perproc_maxunreaped; 
extern int maxasynchio;
...
#ifdef	MULTIPROCESSOR
	klock_require(__FILE__, __LINE__, "lwpschedule");
#endif	MULTIPROCESSOR
	if (runthreads) {
		(void) splx(s);
		return;
...
cminit()
{
	...
	cm_free = cm =
	    ...
	    ((u_int) (sizeof (struct chanmap) * maxasynchio),
	    KMEM_SLEEP);
	...
	thread_t new;
{
	...
	if ((tp->p_flag & SLOAD) == 0)
		(void) swapin(tp);
#ifdef	MULTIPROCESSOR
	hat_map_percpu(cntxt->unix_uarea);
#endif	MULTIPROCESSOR
	...
	register lwp_t *lwp;
{
	struct proc *pp;
	unsigned asmid;
	extern unsigned proc_as_mid();

	pp = (struct proc *)lwp->lwp_ref;
	if ((pp->p_flag & SLOAD) == 0)
		return 1;	/* proc swapped, ok to run LWP */
	asmid = proc_as_mid(pp);
	if (asmid == 0)
		return 1;	/* address space inactive, ok to run. */
	if (asmid == (cpuid|8))
		return 1;	/* address space running here, ok to run. */

	force_lwps |= 1<<(asmid&3);
...
lwp_setref(lwp)
	register lwp_t *lwp;
{
	lwp->lwp_ref = (caddr_t)masterprocp;
}
#endif	MULTIPROCESSOR
