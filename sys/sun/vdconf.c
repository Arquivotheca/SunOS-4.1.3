...
extern char *strcpy();
#ifndef sun4c
extern int vd_checkirq();
extern void vd_resetirq();
#endif sun4c
...
int has_ufsmounted();
#endif
#if !defined(sun386) && !defined(OPENPROMS)
struct vec *allocvec();
void freevec();
#endif
...
#else
	status = vd_configdrv(vdp, vdl->vdldrv_mb_ctlr,
			vdl->vdldrv_mb_driver, vdl->vdldrv_mb_device,
			vdl->vdldrv_numctlrs, vdl->vdldrv_numdevs);
#endif OPENPROMS
	if (status != 0)
		return (status);
...
#else
		(void) vd_unconfigdrv(vdp, vdl->vdldrv_mb_ctlr,
			vdl->vdldrv_mb_driver, vdl->vdldrv_mb_device,
			vdl->vdldrv_numctlrs, vdl->vdldrv_numdevs);
#endif OPENPROMS
	}
...
#else
		status = vd_unconfigdrv(vdp, vdl->vdldrv_mb_ctlr,
			vdl->vdldrv_mb_driver, vdl->vdldrv_mb_device,
			vdl->vdldrv_numctlrs, vdl->vdldrv_numdevs);
#endif OPENPROMS
	}
...
#else
	return (vd_configdrv(vdp, vdl->vdlnet_mb_ctlr,
	...
	struct vdioctl_unload *vdi;
{
	...
	if (status == 0) {
		...
		status = vd_unconfigdrv(vdp, vdl->vdldrv_dev_ops);
#else
		status = vd_unconfigdrv(vdp, vdl->vdlnet_mb_ctlr,
			vdl->vdlnet_mb_driver, vdl->vdlnet_mb_device,
			vdl->vdlnet_numctlrs, vdl->vdlnet_numdevs);
#endif OPENPROMS
	}
	...
	struct vddrv *vdp;
{
	 ...
	 */
#ifdef OPENPROMS
	s2 = strcpy(ifnam, vdp->vdd_vdtab->vdlnet_dname);
#else
	s2 = strcpy(ifnam, vdp->vdd_vdtab->vdlnet_mb_driver->mdr_dname);
#endif OPENPROMS
	...
	*s2 = '\0';		/* terminate the string */
	 ...
	 */
	if ((ifp = ifunit(ifnam, strlen(ifnam))) == NULL) {
 ...
 */
int
vd_configdrv(vdp, mc, mdr, md, numctlrs, numdevs)
	struct vddrv *vdp;
	struct mb_ctlr *mc;
	...
	int numctlrs, numdevs;
{
	...
	if (vd_checkdrvlinkage(vdp, mc, mdr, md, numctlrs, numdevs) < 0)
		return (EINVAL);	/* bad linkage structure */
...
int
vd_unconfigdrv(vdp, mc, mdr, md, numctlrs, numdevs)
	struct vddrv *vdp;
	struct mb_ctlr *mc;
	struct mb_driver *mdr;
	struct mb_device *md;
...
int
vd_checkdrvlinkage(vdp, mc, mdr, md, numctlrs, numdevs)
	register struct vddrv *vdp;
	struct mb_ctlr *mc;
	struct mb_driver *mdr;
	struct mb_device *md;
	...
	int numctlrs;
{
	...
	for (mcend = &mc[numctlrs]; mc < mcend; mc++) {
		...
		for (v = mc->mc_intr; v != NULL && v->v_func != NULL; v++) {
			status = vd_checkirq(v, mc->mc_intpri, mc->mc_space);
			if (status != 0)
				break;
		}
	...
	int numctlrs;
{
	...
	for (mcend = &mc[numctlrs]; mc < mcend; mc++) {
		...
		for (v = mc->mc_intr; v != NULL && v->v_func != NULL; v++)
			vd_resetirq(v, mc->mc_intpri, mc->mc_space);
	...
	int numdevs;
{
	...
	"mb_device struct has md_ctlr == %d., but there are no controllers\n",
			...
			if (mbconfdev(md, &mc[md->md_ctlr], mdr) == -1)
	...
	int numdevs;
{
	...
	for (; md < mdend; md++) {
		for (v = md->md_intr; v != NULL && v->v_func != NULL; v++)
			vd_resetirq(v, md->md_intpri, md->md_space);
