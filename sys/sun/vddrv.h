...
#define	VDMAGIC_MBDRV	(VDMAGIC_BASE + 0x8)	/* sun4m only */
#define	VDMAGIC_MBNET	(VDMAGIC_BASE + 0x9)	/* sun4m only */
...
#ifdef sun4m
/*
 * Ordering of structure elements changed so that existing devinfo drivers
 * need no modification to their vdldrv structures to run on sun4m.
 * mb drivers will need to add the dummy dev_ops entry, and change the
 * ordering of the structure elements in the driver to conform to this.
 */
struct vdldrv {
	int			 Drv_magic;
	char			*Drv_name;
...
#else /* sun4m */
/* 
 * Nothing changes for devinfo style drivers on sun4c, or for
 * mb style drivers on sun4.
 */
struct vdldrv {
	int			 Drv_magic;
	char			*Drv_name;
...
#ifdef sun4m
/*
 * See comments above for vdldrv structure.
 */
struct vdlnet {
	int			 Net_magic;
	char			*Net_name;
